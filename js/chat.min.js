class UltralyticsChat {
  constructor(t = {}) {
    const e = (t, e, s) => t?.[e] ?? s;
    (this.config = {
      apiUrl: e(t, "apiUrl", "https://chat-885297101091.us-central1.run.app/api/chat"),
      maxMessageLength: e(t, "maxMessageLength", 1e4),
      branding: {
        name: e(t.branding, "name", "Ultralytics AI"),
        tagline: e(t.branding, "tagline", "Ask anything about Ultralytics, YOLO, and more"),
        logo: e(
          t.branding,
          "logo",
        ...
        logoUrl: e(t.branding, "logoUrl", "https://www.ultralytics.com"),
        pillText: e(t.branding, "pillText", "Ask AI"),
      },
      theme: {
        primary: e(t.theme, "primary", "#042AFF"),
        dark: e(t.theme, "dark", "#111F68"),
        accent: e(t.theme, "accent", e(t.theme, "yellow", "#E1FF25")),
        text: e(t.theme, "text", "#0b0b0f"),
      },
      welcome: {
        title: e(t.welcome, "title", "Hello 👋"),
        message: e(t.welcome, "message", "I'm an AI assistant trained on Ultralytics documentation - ask me anything!"),
        chatExamples: e(t.welcome, "chatExamples") ??
          e(t.welcome, "examples") ?? [
            "What's new in YOLO11?",
            "How do I get started with YOLO?",
            "Tell me about Enterprise Licensing",
          ],
        searchExamples: e(t.welcome, "searchExamples", [
          "YOLO quickstart",
      ...
      ui: {
        placeholder: e(t.ui, "placeholder", "Ask anything…"),
        copyText: e(t.ui, "copyText", "Copy thread"),
        downloadText: e(t.ui, "downloadText", "Download thread"),
        clearText: e(t.ui, "clearText", "New chat"),
      },
    }),
      ...
      (this.feedbackUrl = e(
        ...
        (() => {
          ...
          return `${this.apiUrl.replace(/\/$/, "")}/feedback`;
        })(),
      )),
      (this.messages = []),
      (this.isOpen = !1),
  ...
  getGroupIndex(t) {
    if (!t) return null;
    const e = Number.parseInt(t.dataset?.messageIndex ?? "", 10);
  ...
  on(t, e, s) {
    t &&
      ...
      this.listeners.get(t).push({ ev: e, fn: s }));
  }
  el(t, e = "", s = "") {
    const a = document.createElement(t);
    return e && (a.className = e), s && (a.innerHTML = s), a;
  }
  showCopySuccess(t) {
    t &&
      (t.__successTimeout && clearTimeout(t.__successTimeout),
      t.dataset.successOriginal || (t.dataset.successOriginal = t.innerHTML),
      (t.innerHTML = this.icon("check")),
      t.classList.add("success"),
      (t.__successTimeout = setTimeout(() => {
        t.dataset.successOriginal && ((t.innerHTML = t.dataset.successOriginal), delete t.dataset.successOriginal),
          t.classList.remove("success"),
          (t.__successTimeout = null);
      }, 1500)));
  }
  flashTooltip(t, e) {
    const s = this.refs.tooltip;
    if (!t || !s) return;
    ...
    (s.textContent = e),
      ...
      s.__timer && clearTimeout(s.__timer),
      (s.__timer = setTimeout(() => {
        s.classList.remove("show"), (s.__timer = null);
      }, 1600));
  }
  getPageContext() {
    return {
      ...
      description: ((t = "description"), document.querySelector(`meta[name="${t}"]`)?.content || ""),
  ...
  saveSessionId(t) {
    try {
      localStorage.setItem("ult-chat-session", t);
    } catch {
  ...
  loadHighlightJS() {
    ...
    const t = this.el("link");
    (t.rel = "stylesheet"), (t.id = "hljs-theme"), document.head.appendChild(t);
    const e = this.el("script");
    (e.src = "https://cdn.jsdelivr.net/gh/highlightjs/cdn-release@11.9.0/build/highlight.min.js"),
      (e.onload = () => window.hljs?.configure({ ignoreUnescapedHTML: !0 })),
      document.head.appendChild(e);
  }
  highlight(t) {
    window.hljs &&
      t?.querySelectorAll("pre code").forEach((t) => {
        if (!t.dataset.highlighted) {
          const e = [...t.classList].find((t) => t.startsWith("lang-"))?.replace("lang-", "");
          e && t.classList.add(`language-${e}`), window.hljs.highlightElement(t);
        }
      });
  }
  ensureViewport() {
    let t = document.querySelector('meta[name="viewport"]');
    t || ((t = document.createElement("meta")), (t.name = "viewport"), document.head.appendChild(t)),
      t.content.includes("maximum-scale") ||
  ...
  watchForRemoval() {
    const t = [
      ...
      e = new MutationObserver(() =>
        t.forEach(({ element: t, parent: e }) => {
          const s = t();
          s?.parentNode !== e && e.appendChild(s);
        }),
      );
    e.observe(document.head, { childList: !0 }), e.observe(document.body, { childList: !0 }), (this.domObserver = e);
  }
  destroy() {
    this.toggle(!1),
      this.inputDebounceTimer && clearTimeout(this.inputDebounceTimer),
      this.domObserver?.disconnect(),
      this.listeners.forEach((t, e) => t.forEach(({ ev: t, fn: s }) => e.removeEventListener(t, s))),
  ...
  syncThemeWithSite() {
    const t = document.documentElement,
      ...
      s = () => {
        t.hasAttribute("data-theme") || t.setAttribute("data-theme", e.matches ? "dark" : "light");
        const s = document.getElementById("hljs-theme");
        if (s) {
          const e = "dark" === t.getAttribute("data-theme");
          s.href = `https://cdn.jsdelivr.net/gh/highlightjs/cdn-release@11.9.0/build/styles/github${e ? "-dark" : ""}.min.css`;
        }
      };
    s(), e.addEventListener("change", s);
  }
  createStyles() {
    const { primary: t, dark: e, accent: s, text: a } = this.config.theme;
  ...
  setExamples(t) {
    this.refs.examples &&
      ((this.refs.examples.innerHTML = t
        .map((t) => `<button class="ult-example" data-q="${this.esc(t)}">${this.esc(t)}</button>`)
        .join("")),
      this.qsa(".ult-example", this.refs.examples).forEach((t) =>
        this.on(t, "click", () => {
          this.sendMessage(t.dataset.q);
        }),
      ));
  }
  attachEvents() {
    ...
    this.setupTooltips(),
      this.on(this.refs.pill, "click", () => this.toggle(!0, "chat")),
      this.on(this.refs.backdrop, "click", () => this.toggle()),
      this.on(this.qs(".ult-chat-close", t), "click", () => this.toggle()),
      this.on(this.qs(".ult-chat-clear", t), "click", () => this.clearSession()),
      this.on(this.qs(".ult-chat-copy", t), "click", () => this.copyThread()),
      this.on(this.qs(".ult-chat-download", t), "click", () => this.downloadThread()),
      this.on(this.refs.messages, "scroll", () => {
        const t = this.refs.messages,
          e = t.scrollTop;
        (this.autoScroll = e >= (this.lastScrollTop || 0) && t.scrollHeight - e - t.clientHeight < 100),
          (this.lastScrollTop = e);
      }),
      this.on(this.refs.input, "input", (t) => {
        const e = t.target;
        (e.style.height = "auto"),
          (e.style.height = Math.min(e.scrollHeight, 140) + "px"),
          e.value.length === this.config.maxMessageLength && this.flashTooltip(e, "⚠️ Message shortened to fit"),
          this.inputDebounceTimer && clearTimeout(this.inputDebounceTimer),
          (this.inputDebounceTimer = setTimeout(() => this.updateComposerState(), 50));
      }),
      this.on(this.refs.send, "click", () => {
        this.isStreaming ? this.stopStreaming() : this.sendMessage(this.refs.input.value.trim());
      }),
      this.on(this.refs.input, "keydown", (t) => {
        "Enter" !== t.key || t.shiftKey || (t.preventDefault(), this.sendMessage(this.refs.input.value.trim())),
          "Escape" === t.key && (t.preventDefault(), this.toggle(!1));
      }),
      this.on(document, "keydown", (t) => {
        this.isOpen && "Escape" === t.key && this.toggle(!1),
          !this.isOpen && t.metaKey && "k" === t.key.toLowerCase() && this.toggle(!0);
      }),
      this.on(this.refs.messages, "mouseover", (t) => {
        const e = t.target.closest("[data-tooltip]");
        if (e && this.refs.tooltip && !e.dataset.tooltipActive) {
          e.dataset.tooltipActive = "true";
          const t = e.getBoundingClientRect();
          (this.refs.tooltip.textContent = e.dataset.tooltip),
            (this.refs.tooltip.style.left = t.left + t.width / 2 + "px"),
            (this.refs.tooltip.style.top = t.top - 8 + "px"),
            this.refs.tooltip.classList.add("show");
        }
      }),
      this.on(this.refs.messages, "mouseout", (t) => {
        const e = t.target.closest("[data-tooltip]");
        !e ||
          (t.relatedTarget && e.contains(t.relatedTarget)) ||
          (delete e.dataset.tooltipActive, this.refs.tooltip?.classList.remove("show"));
      }),
      this.on(this.refs.messages, "click", (t) => {
        if (t.target.closest(".ult-code-copy")) {
          const e = t.target.closest(".ult-code-copy"),
            s = e.previousElementSibling?.querySelector("code")?.textContent || "";
          navigator.clipboard
            ?.writeText(s)
            .then(() => this.showCopySuccess(e))
            .catch(console.error);
        }
        const e = t.target.closest("[data-action]");
        if (e) {
          const t = e.dataset.action,
            s = e.closest(".ult-message-group");
          if ("copy" === t) {
            const t = [...this.qsa(".ult-message-group[data-role='assistant']", this.refs.messages)].indexOf(s),
              a = this.messages.filter((t) => "assistant" === t.role),
              i = a[t]?.content;
            i &&
              navigator.clipboard
                ?.writeText(i)
                ?.then(() => this.showCopySuccess(e))
                .catch(console.error);
          } else if ("like" === t || "dislike" === t)
            this.feedback("like" === t ? "up" : "down"), this.showCopySuccess(e);
          else if ("retry" === t) this.retryLast();
          else if ("edit" === t) {
            const t = s.querySelector(".ult-message"),
              a = this.getGroupIndex(s);
            if (t && null !== a) {
              let s = t.textContent.trim();
              if (!s) return;
              if (this.isStreaming) return void this.flashTooltip(e, "⚠️ Finish generating first");
              (s = this.trimMessage(s, e)),
                (t.textContent = s),
                this.editAndRestart(a, s).then((t) => {
                  t && this.showCopySuccess(e);
                });
            }
          }
        }
      }),
      this.on(this.refs.messages, "keydown", (t) => {
        const e = t.target.closest(".ult-message[contenteditable='true']");
        if (e)
          if ("Enter" !== t.key || t.shiftKey)
            "Escape" === t.key && (t.preventDefault(), (e.textContent = e.dataset.originalContent || ""), e.blur());
          else {
            t.preventDefault();
            const s = e.closest(".ult-message-group"),
              a = this.getGroupIndex(s);
            if (null !== a) {
              if (this.isStreaming) return void this.flashTooltip(e, "⚠️ Finish generating first");
              let t = e.textContent.trim();
              if (!t) return;
              (t = this.trimMessage(t, e)), (e.textContent = t), this.editAndRestart(a, t);
            }
          }
      }),
      this.on(this.refs.messages, "focusin", (t) => {
        const e = t.target.closest(".ult-message[contenteditable='true']");
        e && e.classList.add("ult-message-editing");
      }),
      this.on(this.refs.messages, "focusout", (t) => {
        const e = t.target.closest(".ult-message[contenteditable='true']");
        e && e.classList.remove("ult-message-editing");
      }),
      this.on(this.refs.messages, "input", (t) => {
        const e = t.target.closest(".ult-message[contenteditable='true']");
        if (!e) return;
        const s = this.trimMessage(e.textContent || "", e);
        s !== (e.textContent || "") && (e.textContent = s);
      });
  }
  toggle(t = null, e = null) {
    const s = null === t ? !this.isOpen : !!t;
    if ((e && (this.mode = e), s === this.isOpen && !e)) return;
    this.isOpen = s;
    const a = this.refs.modal,
      i = this.refs.backdrop,
      o = this.refs.pill;
    if (s)
      a && (a.style.display = "flex"),
        i && (i.style.display = "block"),
        requestAnimationFrame(() => {
          a?.classList.add("open"), i?.classList.add("open"), o?.classList.add("hidden");
        });
    else {
      a?.classList.remove("open"), i?.classList.remove("open"), o?.classList.remove("hidden");
      const t = () => {
        this.isOpen || (a && (a.style.display = "none"), i && (i.style.display = "none"));
      };
      a?.addEventListener("transitionend", t, { once: !0 }),
        i?.addEventListener("transitionend", t, { once: !0 }),
        setTimeout(t, 250);
    }
    s
      ? ((this.scrollY = window.scrollY),
        ...
        this.messages.length || this.showWelcome(!0),
        this.updateComposerState(),
        setTimeout(() => {
          this.refs.input?.focus(), this.refs.input?.click();
        }, 100))
      : (document.body.classList.remove("ult-modal-open"),
        (document.body.style.top = ""),
        window.scrollTo(0, this.scrollY),
        this.isStreaming && this.stopStreaming());
  }
  updateUIForMode() {
    if (!this.refs.modal) return;
    const t = this.qs(".ult-subtle", this.refs.modal);
    if (((this.refs.modal.dataset.mode = this.mode), "search" === this.mode))
      this.refs.input && (this.refs.input.placeholder = "Search for..."),
        t &&
          (t.innerHTML = `<strong style="color: ${this.config.theme.primary}; font-weight: 700;">SEARCH</strong> · Find answers in our docs and guides`),
        this.refs.messages && (this.refs.messages.innerHTML = ""),
        this.refs.welcome &&
      ...
      this.refs.input && (this.refs.input.placeholder = this.config.ui.placeholder),
        t && (t.textContent = this.config.branding.tagline);
      const { title: e, message: s, chatExamples: a } = this.config.welcome;
      this.refs.welcome && (this.refs.welcome.innerHTML = `<h1>${this.esc(e)}</h1><p>${s}</p>`),
        this.setExamples(a || []),
        this.renderChatHistory();
    }
  }
  showWelcome(t) {
    this.refs.welcome && (this.refs.welcome.style.display = t ? "block" : "none"),
      this.refs.examples && (this.refs.examples.style.display = t ? "flex" : "none"),
      this.refs.modal && this.refs.modal.classList.toggle("welcome-mode", t);
  }
  renderChatHistory() {
    if (!this.refs.messages) return;
    if (((this.refs.messages.innerHTML = ""), !this.messages.length)) return void this.showWelcome(!0);
    this.showWelcome(!1);
    const t = this.autoScroll;
    (this.autoScroll = !1),
      this.messages.forEach((t, e) => this.addMessageToUI(t.role, t.content, e)),
      (this.autoScroll = t),
      (this.refs.messages.scrollTop = this.refs.messages.scrollHeight);
  }
  setupTooltips() {
    this.qsa("[data-tooltip]", this.refs.modal).forEach((t) => {
      this.on(t, "mouseenter", (t) => {
        const e = this.refs.tooltip;
        if (!e) return;
        const s = t.currentTarget.getBoundingClientRect();
        (e.textContent = t.currentTarget.dataset.tooltip),
          (e.style.left = s.left + s.width / 2 + "px"),
          (e.style.top = s.top - 8 + "px"),
          e.classList.add("show");
      }),
        this.on(t, "mouseleave", () => this.refs.tooltip?.classList.remove("show"));
    });
  }
  swapSendIcon(t) {
    const e = this.qs(".ult-icon-swap", this.refs.send);
    if (!e || e.dataset.icon === t) return;
    (e.innerHTML = this.icon(t)), (e.dataset.icon = t);
    const s = "square" === t && this.isStreaming ? "Stop" : "arrowUp" === t ? "Send" : "Ready";
    this.refs.send.setAttribute("aria-label", s), (this.refs.send.dataset.tooltip = s);
  }
  updateComposerState() {
    const t = !!this.refs.input?.value.trim().length;
    this.swapSendIcon(this.isStreaming ? "square" : t ? "arrowUp" : "square");
  }
  trimMessage(t, e = null) {
    const s = this.config.maxMessageLength;
    return !t || t.length <= s ? t || "" : (e && this.flashTooltip(e, "⚠️ Message shortened to fit"), t.slice(0, s));
  }
  scrollToBottom() {
    this.autoScroll &&
      this.refs.messages &&
  ...
  formatThread() {
    return this.messages
      .map((t) => `${"user" === t.role ? "You" : this.config.branding.name}: ${t.content}`)
      .join("\n\n---\n\n");
  }
  copyThread() {
  ...
  async feedback(t) {
    const e = "up" === t,
      s = this.messages.filter((t) => "user" === t.role).length;
    if (!s) return void console.warn("feedback ignored: no user messages yet");
    const a = s - 1;
    try {
      const t = await fetch(this.feedbackUrl, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ session_id: this.sessionId, query_index: a, vote: e }),
      });
      if (!t.ok) throw new Error(`feedback failed with status ${t.status}`);
    } catch (t) {
      console.warn("feedback failed", t);
    }
  }
  retryLast() {
    const t = [...this.messages].reverse().findIndex((t) => "assistant" === t.role);
    if (-1 === t) return;
    const e = this.messages.length - 1 - t,
      s = this.messages[e - 1];
    s && "user" === s.role && this.editAndRestart(e - 1, s.content);
  }
  async editAndRestart(t, e) {
    if (this.isStreaming) return !1;
    if (!Number.isInteger(t) || t < 0 || t >= this.messages.length) return !1;
    const s = this.messages[t];
    if ("user" !== s.role) return !1;
    (e = this.trimMessage(e)), (s.content = e), (this.messages = this.messages.slice(0, t + 1));
    const a = this.qs(`.ult-message-group[data-message-index="${t}"]`, this.refs.messages),
      i = a?.querySelector(".ult-message");
    i && (i.dataset.originalContent = e);
    return (
      this.qsa(".ult-message-group", this.refs.messages).forEach((e) => {
        const s = this.getGroupIndex(e);
        (null === s || s > t) && e.remove();
      }),
      this.refs.tooltip && this.refs.tooltip.classList.remove("show"),
      await this.sendMessage(e, !1, t),
      !0
    );
  }
  downloadThread() {
    const { name: t } = this.config.branding,
      e = new Blob([this.formatThread()], { type: "text/plain" }),
      s = URL.createObjectURL(e),
      a = this.el("a");
    (a.href = s),
      (a.download = `${t.toLowerCase().replace(/\s+/g, "-")}-chat-${new Date().toISOString().slice(0, 10)}.txt`),
      a.click(),
      URL.revokeObjectURL(s);
  }
  clearSession() {
    (this.messages = []), (this.sessionId = null);
    try {
  ...
  createThinking(t = "Thinking") {
    const e = this.el("div", "ult-message assistant ult-message-thinking"),
      s = this.el(
        "p",
        "ult-thinking",
        `<span class="ult-thinking-word">${t}</span><span class="ult-typing"><span></span><span></span><span></span></span><span class="ult-thinking-time">(0.0s)</span>`,
      );
    e.appendChild(s);
    const a = this.qs(".ult-thinking-time", e),
      i = performance.now(),
      o = setInterval(() => {
        a && (a.textContent = `(${((performance.now() - i) / 1e3).toFixed(1)}s)`);
      }, 100);
    return { el: e, clear: () => clearInterval(o) };
  }
  async performSearch(t) {
    if (!this.refs.messages) return;
    this.refs.messages.innerHTML = "";
    const { el: e, clear: s } = this.createThinking("Searching");
    this.refs.messages.appendChild(e);
    try {
      const a = this.apiUrl.replace(/\/chat$/, "/search"),
        i = await fetch(a, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      if ((s(), e.remove(), !o.results?.length))
        return void (this.refs.messages.innerHTML =
          '<div class="ult-message">No results found. Try different keywords.</div>');
      this.refs.messages.innerHTML = o.results
        .map((t) => {
          const e = t.text?.length > 150 ? t.text.slice(0, 150) + "..." : t.text || "",
            s = (() => {
              try {
            ...
            a = s ? `https://www.google.com/s2/favicons?sz=32&domain=${encodeURIComponent(s)}` : "",
            i = a ? `<img class="ult-search-result-favicon" src="${a}" alt="" loading="lazy" />` : "",
            o = s ? `<span>${this.esc(s)}</span>` : "";
          return `<div class="ult-search-result"><div class="ult-search-result-title">${i}<a href="${this.esc(t.url)}" target="_blank" rel="noopener">${this.esc(t.title || "")}</a></div><div class="ult-search-result-snippet">${this.esc(e)}</div><div class="ult-search-result-meta"><span class="ult-search-result-score">Match: ${(100 * (t.score || 0)).toFixed(0)}%</span>${o}</div></div>`;
        })
        .join("");
    } catch (t) {
      s(),
        e.remove(),
        this.refs.messages &&
          (this.refs.messages.innerHTML = `<div class="ult-message">Search error: ${this.esc(t.message)}</div>`),
        console.error("Search error:", t);
    }
  }
  async sendMessage(t, e = !0, s = null) {
    if (!t || this.isStreaming || !this.refs.input || !this.refs.messages) return;
    if (!(t = this.trimMessage(t, this.refs.input))) return;
    if ((this.showWelcome(!1), (this.autoScroll = !0), "search" === this.mode))
      return (
        (this.refs.input.value = t),
        (this.refs.input.style.height = "auto"),
        (this.refs.input.style.height = Math.min(this.refs.input.scrollHeight, 140) + "px"),
        await this.performSearch(t),
        void this.refs.input.focus()
      );
    e && (this.messages.push({ role: "user", content: t }), this.addMessageToUI("user", t, this.messages.length - 1)),
    ...
    const a = this.createMessageGroup("assistant", this.messages.length),
      { el: i, clear: o } = this.createThinking();
    a.appendChild(i), this.scrollToBottom(), (this.abortController = new AbortController());
    const n = Number.isInteger(s) && s >= 0 && s < this.messages.length ? s : null;
    try {
      const e = {
      ...
      const r = s.headers.get("X-Session-ID");
      r && !this.sessionId && ((this.sessionId = r), this.saveSessionId(r)), i.remove(), o();
      const l = this.el("div", "ult-message assistant");
      a.appendChild(l);
      const c = s.body?.getReader();
      if (!c) throw new Error("No response stream");
      ...
      const u = () => {
        p ||
          (p = setTimeout(() => {
            (l.innerHTML = this.renderMarkdown(d, !0)), this.highlight(l), this.scrollToBottom(), (p = null);
          }, 30));
      };
      for (;;) {
        const { done: t, value: e } = await c.read();
        if (t) break;
        const s = h.decode(e, { stream: !0 });
        for (const t of s.split("\n")) {
          ...
          if ("[DONE]" !== e)
            try {
              ...
              if (t.content) (d += t.content), u();
              else if (t.error) throw new Error(t.error);
            } catch (t) {
              "Unexpected end of JSON input" !== t.message && console.error("Parse error:", t);
            }
        }
      }
      p && clearTimeout(p),
        (l.innerHTML = this.renderMarkdown(d, !1)),
        this.finalizeAssistantMessage(a),
        this.scrollToBottom(),
        this.messages.push({ role: "assistant", content: d });
    } catch (t) {
      i.remove(), o();
      const e =
          "AbortError" === t.name
        ...
        s = this.el("div", "ult-message assistant", e);
      a.appendChild(s), console.error("Chat error:", t);
    } finally {
      (this.isStreaming = !1),
        (this.abortController = null),
        this.updateComposerState(),
        this.qsa(".ult-message[contenteditable]", this.refs.messages).forEach((t) => (t.contentEditable = "true")),
        this.refs.input?.focus();
    }
  }
  createMessageGroup(t, e = null) {
    if (!this.refs.messages) return null;
    const { name: s, logomark: a } = this.config.branding,
      i = this.el("div", "ult-message-group");
    (i.dataset.role = t), null !== e && (i.dataset.messageIndex = e);
    const o = this.el(
      "div",
      "ult-message-label",
      "assistant" === t
        ? `<img src="${this.esc(a)}" alt="${this.esc(s)}" /><span>${this.esc(s)}</span>`
    ...
    return i.appendChild(o), this.refs.messages.appendChild(i), this.scrollToBottom(), i;
  }
  addMessageToUI(t, e, s = null) {
    const a = this.createMessageGroup(t, s);
    if (!a) return null;
    const i = this.el("div", "ult-message " + ("assistant" === t ? "assistant" : ""), this.renderMarkdown(e));
    return (
      "user" === t &&
        ((i.contentEditable = "true"),
        (i.dataset.originalContent = e),
        i.setAttribute("role", "textbox"),
        i.setAttribute("aria-label", "Edit your message"),
        i.setAttribute("aria-multiline", "true"),
        this.isStreaming && (i.contentEditable = "false")),
      a.appendChild(i),
      "assistant" === t ? this.finalizeAssistantMessage(a) : this.addUserMessageActions(a),
      i
    );
  }
  addMessageActions(t) {
    if (t.querySelector(".ult-message-actions")) return;
    const e = this.el(
      "div",
      "ult-message-actions",
  ...
  addUserMessageActions(t) {
    if (t.querySelector(".ult-user-message-actions")) return;
    const e = this.el(
      "div",
  ...
  finalizeAssistantMessage(t) {
    if (!t || "assistant" !== t.dataset.role) return;
    const e = t.querySelector(".ult-message.assistant");
    e && this.highlight(e), this.addMessageActions(t);
  }
  escapeHtml(t) {
    const e = this.el("div");
    return (e.textContent = t), e.innerHTML;
  }
  renderMarkdown(t, e = !1) {
    const s = (t) =>
        t
          .replace(/&/g, "&amp;")
          .replace(/</g, "&lt;")
    ...
    const d = () => {
      ...
      u = () => {
        l && ((i += "ol" === r ? "</ol>" : "</ul>"), (l = !1), (r = null));
      },
      m = () => {
        c && ((i += "</blockquote>"), (c = !1));
      };
    for (let t of a) {
      const a = t.match(/^\s*```(\w+)?\s*$/);
      if (a) {
        o
          ? ((i += e
              ? "</code></pre></div>"
          ...
          : (d(),
            ...
            (o = !0),
            (n = t.search(/\S/)),
            (i += `<div class="ult-code-block"><pre><code class="lang-${s(a[1] || "")}">`));
        continue;
      }
      if (o) {
        const e = t.slice(n);
      ...
      const h = /^>\s?(.*)$/.exec(t);
      let g;
      h ? (c || (d(), u(), (i += "<blockquote>"), (c = !0)), (t = h[1])) : m(),
        (g = t.match(/^\s*([-*+])\s+(.+)$/))
          ? ((l && "ul" === r) || (d(), u(), (i += "<ul>"), (l = !0), (r = "ul")),
            (i += `<li>${this.renderInline(g[2])}</li>`))
          : (g = t.match(/^\s*(\d+)\.\s+(.+)$/))
            ? ((l && "ol" === r) || (d(), u(), (i += "<ol>"), (l = !0), (r = "ol")),
              (i += `<li>${this.renderInline(g[2])}</li>`))
            : /^\s*$/.test(t)
              ? (d(), u(), m())
              : (g = t.match(/^###\s+(.+)$/))
                ? (d(), u(), m(), (i += `<h3>${this.renderInline(g[1])}</h3>`))
                : (g = t.match(/^##\s+(.+)$/))
                  ? (d(), u(), m(), (i += `<h2>${this.renderInline(g[1])}</h2>`))
                  : (g = t.match(/^#\s+(.+)$/))
                    ? (d(), u(), m(), (i += `<h1>${this.renderInline(g[1])}</h1>`))
                    : (p(), (i += this.renderInline(t)));
    }
    return d(), u(), m(), i;
  }
  renderInline(t) {
    ...
    return (t = (t = (t = (t = (t = (t = (t = (t = t.replace(
      /`([^`]+)`/g,
      (t, s) => (e.push(s), `@@ULTCODE${e.length - 1}@@`),
    )).replace(
      /\[([^\]]+)]\((https?:\/\/[^\s)]+)\)/g,
      (t, e, s) => `<a href="${s}" target="_blank" rel="noopener noreferrer">${e}</a>`,
    )).replace(
      /(?<!href=")(?<!src=")(?<!>)\b(https?:\/\/[^\s<>'")\]]+?)(?=[.,;:!?]*(?:\s|<|'|"|\)|]|$))/g,
      (t) => `<a href="${t}" target="_blank" rel="noopener noreferrer">${t}</a>`,
    )).replace(/\*\*(.+?)\*\*/g, "<strong>$1</strong>")).replace(/__(.+?)__/g, "<strong>$1</strong>")).replace(
      /(?<!\*)\*(?!\*)(.+?)\*(?!\*)/g,
      "<em>$1</em>",
    )).replace(/(?<!_)_(?!_)(.+?)_(?!_)/g, "<em>$1</em>")).replace(
      /@@ULTCODE(\d+)@@/g,
      (t, s) => `<code>${e[s]}</code>`,
    )).replace(/ {2}\n/g, "<br>");
  }
