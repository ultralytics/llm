class UltralyticsChat {
  constructor(t = {}) {
    ((this.config = {
      apiUrl: t.apiUrl || "/api/chat",
      branding: {
        name: t.branding?.name || "AI",
        tagline:
          t.branding?.tagline ||
        ...
        pillText: t.branding?.pillText || "Ask AI",
      },
      theme: {
        primary: t.theme?.primary || "#042AFF",
        dark: t.theme?.dark || "#111F68",
        yellow: t.theme?.yellow || "#E1FF25",
        text: t.theme?.text || "#0b0b0f",
      },
      welcome: {
        title: t.welcome?.title || "Hi!",
        message:
          t.welcome?.message ||
          "I'm an AI assistant trained on documentation, help articles, and other content.<br>Ask me anything about Ultralytics.",
        examples: t.welcome?.examples || [
      ...
      ui: {
        placeholder: t.ui?.placeholder || "Ask anything…",
        copyText: t.ui?.copyText || "Copy thread",
        downloadText: t.ui?.downloadText || "Download thread",
        clearText: t.ui?.clearText || "New chat",
      },
    }),
    ...
    const t = document.documentElement,
      e = () => {
        if (!t.hasAttribute("data-theme")) {
          const e = matchMedia("(prefers-color-scheme: dark)").matches;
          t.setAttribute("data-theme", e ? "dark" : "light");
        }
      };
    e();
    const s = matchMedia("(prefers-color-scheme: dark)"),
      a = () => e();
    try {
  ...
  createStyles() {
    const { primary: t, dark: e, yellow: s, text: a } = this.config.theme,
      n = this.el(
        "style",
        "",
  ...
  createUI() {
    const {
      ...
      { title: i, message: r, examples: o } = this.config.welcome,
      {
        placeholder: l,
        copyText: c,
        downloadText: h,
  ...
  setExamples(t) {
    ((this.refs.examples.innerHTML = t
      .map((t) => `<button class="ult-example" data-q="${t}">${t}</button>`)
      .join("")),
      this.qsa(".ult-example", this.refs.examples).forEach((t) =>
        this.on(t, "click", () => this.sendMessage(t.dataset.q)),
      ));
  }
  attachEvents() {
    ...
    (this.on(this.refs.pill, "click", () => this.toggle(!0, "chat")),
      ...
      this.on(this.qs(".ult-chat-clear", t), "click", () =>
        this.clearSession(),
      ),
      this.on(this.qs(".ult-chat-copy", t), "click", () => this.copyThread()),
      this.on(this.qs(".ult-chat-download", t), "click", () =>
        this.downloadThread(),
      ),
      this.on(this.refs.messages, "scroll", () => {
        const t = this.refs.messages;
        this.autoScroll = t.scrollHeight - t.scrollTop - t.clientHeight < 100;
      }),
      ...
      this.on(this.refs.input, "keydown", (t) => {
        ("Enter" !== t.key ||
          t.shiftKey ||
          (t.preventDefault(), this.sendMessage(this.refs.input.value.trim())),
      ...
      this.on(this.qs(".ult-act-copy", t), "click", () =>
        this.copyLastAssistant(),
      ),
      this.on(this.qs(".ult-act-like", t), "click", () => this.feedback("up")),
      this.on(this.qs(".ult-act-dislike", t), "click", () =>
        this.feedback("down"),
      ),
      this.on(this.qs(".ult-act-share", t), "click", () => this.copyThread()),
      this.on(this.qs(".ult-act-retry", t), "click", () => this.retryLast()));
  }
  toggle(t = null, e = null) {
    const s = null === t ? !this.isOpen : !!t;
    ((this.isOpen = s),
      e && (this.mode = e),
      this.refs.modal.classList.toggle("open", s),
      this.refs.backdrop.classList.toggle("open", s),
      this.refs.pill.classList.toggle("hidden", s),
      (document.documentElement.style.overflow = s ? "hidden" : ""),
      s
  ...
  updateUIForMode() {
    ...
    if (((this.refs.modal.dataset.mode = this.mode), "search" === this.mode))
      ((this.refs.input.placeholder = "Search for..."),
        ...
        (this.refs.welcome.innerHTML =
          "<p>Enter keywords to find relevant documentation, guides, and resources</p>"),
        this.setExamples([
          "YOLO quickstart",
      ...
      ((this.refs.input.placeholder = this.config.ui.placeholder),
        (t.textContent = this.config.branding.tagline),
        (this.qs(".ult-actions", this.refs.modal).style.display = ""));
      const { title: e, message: s, examples: a } = this.config.welcome;
      ((this.refs.welcome.innerHTML = `<h1>${e}</h1><p>${s}</p>`),
        this.setExamples(a),
        this.renderChatHistory());
    }
  }
  showWelcome(t) {
    ((this.refs.welcome.style.display = t ? "block" : "none"),
      (this.refs.examples.style.display = t ? "flex" : "none"));
  }
  renderChatHistory() {
    if (((this.refs.messages.innerHTML = ""), !this.messages.length))
      return void this.showWelcome(!0);
    this.showWelcome(!1);
    const t = this.autoScroll;
    ((this.autoScroll = !1),
      this.messages.forEach((t) => this.addMessageToUI(t.role, t.content)),
      (this.autoScroll = t),
  ...
  swapSendIcon(t) {
    ...
    e.dataset.icon !== t &&
      ((e.innerHTML = this.icon(t)),
      (e.dataset.icon = t),
      (this.refs.send.title =
        "square" === t && this.isStreaming
          ? "Stop"
          : "arrowUp" === t
            ? "Send"
            : "Ready"),
  ...
  updateComposerState() {
    ...
    this.isStreaming
      ...
      : this.swapSendIcon(t ? "arrowUp" : "square");
  }
  scrollToBottom() {
  ...
  copyThread() {
    const t = this.messages
      .map(
        (t) =>
          `${"user" === t.role ? "You" : this.config.branding.name}: ${t.content}`,
      )
      .join("\n\n---\n\n");
    navigator.clipboard.writeText(t).catch(console.error);
  }
  copyLastAssistant() {
    const t = [...this.messages].reverse().find((t) => "assistant" === t.role);
    t && navigator.clipboard.writeText(t.content).catch(console.error);
  }
  feedback(t) {
    console.log("feedback:", t);
  }
  retryLast() {
    const t = [...this.messages].reverse().find((t) => "user" === t.role);
    t && this.sendMessage(t.content);
  }
  downloadThread() {
    const { name: t } = this.config.branding,
      e = this.messages
        .map((e) => `${"user" === e.role ? "You" : t}: ${e.content}`)
        .join("\n\n---\n\n"),
      s = new Blob([e], { type: "text/plain" }),
      a = URL.createObjectURL(s),
      n = this.el("a");
    ((n.href = a),
      (n.download = `${t.toLowerCase().replace(/\s+/g, "-")}-chat-${new Date().toISOString().slice(0, 10)}.txt`),
      n.click(),
  ...
  async performSearch(t) {
    ...
    const s = this.qs(".ult-thinking-time", e),
      a = performance.now(),
      n = setInterval(() => {
        s.textContent = `(${((performance.now() - a) / 1e3).toFixed(1)}s)`;
      }, 100);
    try {
      const s = this.apiUrl.replace(/\/chat$/, "/search"),
        a = await fetch(s, {
          method: "POST",
      ...
      if ((clearInterval(n), e.remove(), !i.results?.length))
        return void (this.refs.messages.innerHTML =
          '<div class="ult-message">No results found. Try different keywords.</div>');
      this.refs.messages.innerHTML = i.results
        .map((t) => {
          const e = t.text.length > 150 ? t.text.slice(0, 150) + "..." : t.text;
          let s = "";
          try {
            s = new URL(t.url).hostname;
          } catch {
            s = t.url;
          }
          const a = s
              ? `https://www.google.com/s2/favicons?sz=32&domain=${encodeURIComponent(s)}`
              : "",
            n = a
              ? `<img class="ult-search-result-favicon" src="${a}" alt="" loading="lazy" />`
              : "",
            i = s ? `<span>${this.escapeHtml(s)}</span>` : "";
      ...
      (clearInterval(n),
        e.remove(),
        (this.refs.messages.innerHTML = `<div class="ult-message">Search error: ${t.message}</div>`),
  ...
  async sendMessage(t) {
    ...
    (this.messages.push({ role: "user", content: t }),
      ...
      (this.isStreaming = !0),
      this.updateComposerState());
    const e = this.createMessageGroup("assistant"),
      s = this.el(
        "div",
        "ult-thinking",
    ...
    e.appendChild(s);
    const a = this.qs(".ult-thinking-time", s),
      n = performance.now(),
      i = setInterval(() => {
        a.textContent = `(${((performance.now() - n) / 1e3).toFixed(1)}s)`;
      }, 100);
    (this.scrollToBottom(), (this.abortController = new AbortController()));
    try {
      const a = await fetch(this.apiUrl, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      if (!a.ok) throw new Error(`HTTP ${a.status}`);
      const n = a.headers.get("X-Session-ID");
      (n && !this.sessionId && ((this.sessionId = n), this.saveSessionId(n)),
        s.remove(),
        clearInterval(i));
      const r = this.el("div", "ult-message assistant");
      e.appendChild(r);
        ...
        for (const t of s.split("\n")) {
          ...
          if ("[DONE]" !== e)
            try {
              ...
              if (t.content)
                ((c += t.content),
                  (r.innerHTML = this.renderMarkdown(c)),
                  this.scrollToBottom());
              else if (t.error) throw new Error(t.error);
            } catch (t) {
              "Unexpected end of JSON input" !== t.message &&
      ...
      (e.appendChild(a), console.error("Chat error:", t));
    } finally {
      ((this.isStreaming = !1),
        (this.abortController = null),
        this.updateComposerState(),
  ...
  createMessageGroup(t) {
    const { name: e, logomark: s } = this.config.branding,
      a = this.el("div", "ult-message-group"),
      n = this.el(
        "div",
        "ult-message-label",
        "assistant" === t
          ? `<img src="${s}" alt="${e}" /><span>${e}</span>`
    ...
    return (
      a.appendChild(n),
      this.refs.messages.appendChild(a),
      this.scrollToBottom(),
      a
    );
  }
  addMessageToUI(t, e) {
    const s = this.createMessageGroup(t),
      a = this.el(
        "div",
        "ult-message " + ("assistant" === t ? "assistant" : ""),
        this.renderMarkdown(e),
      );
  ...
  renderMarkdown(t) {
    ...
    for (let t of s) {
      ...
      const c = /^>\s?(.*)$/.exec(t);
      let m;
      (c ? (l || (h(), p(), (a += "<blockquote>"), (l = !0)), (t = c[1])) : u(),
        (m = t.match(/^\s*([-*+])\s+(.+)$/))
          ? ((o && "ul" === r) ||
              (h(), p(), (a += "<ul>"), (o = !0), (r = "ul")),
            (a += `<li>${this.renderInline(m[2])}</li>`))
          : (m = t.match(/^\s*(\d+)\.\s+(.+)$/))
            ? ((o && "ol" === r) ||
                (h(), p(), (a += "<ol>"), (o = !0), (r = "ol")),
              (a += `<li>${this.renderInline(m[2])}</li>`))
            : /^\s*$/.test(t)
              ? (h(), p(), u())
              : (m = t.match(/^###\s+(.+)$/))
                ? (h(), p(), u(), (a += `<h3>${this.renderInline(m[1])}</h3>`))
                : (m = t.match(/^##\s+(.+)$/))
                  ? (h(),
                    p(),
                    u(),
                    (a += `<h2>${this.renderInline(m[1])}</h2>`))
                  : (m = t.match(/^#\s+(.+)$/))
                    ? (h(),
                      p(),
                      u(),
                      (a += `<h1>${this.renderInline(m[1])}</h1>`))
                    : (d(), (a += this.renderInline(t))));
    }
    return (h(), p(), u(), a);
  }
  renderInline(t) {
    return t
      ? (t = (t = (t = (t = (t = (t = t
        ...
        )).replace(/(\*\*|__)(.*?)\1/g, "<strong>$2</strong>")).replace(
          /(\*|_)([^*_]+)\1/g,
          "<em>$2</em>",
        )).replace(/`([^`]+)`/g, "<code>$1</code>")).replace(/ {2}\n/g, "<br>")
      : "";
  }
}
